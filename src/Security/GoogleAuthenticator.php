...
class GoogleAuthenticator extends OAuth2Authenticator
{
    ...
    public function authenticate(Request $request): Passport
    {
        ...
        $client->getOAuth2Provider()->setHttpClient(new \GuzzleHttp\Client(['verify' => false]));
        ...
        return new SelfValidatingPassport(
            new UserBadge($accessToken->getToken(), function () use ($accessToken, $client) {
                ...
                // Set missing non-nullable fields using Google data if available
                $phone = $userData['phone_number'] ?? $userData['phone'] ?? '00000000';
                $user->setTelephone($phone);
                
                $locale = $googleUser->getLocale() ?? 'Inconnue';
                $user->setNationalite($locale);
                
                $birthday = $userData['birthdate'] ?? $userData['birthday'] ?? null;
                $dob = $birthday ? new \DateTime($birthday) : new \DateTime('2000-01-01');
